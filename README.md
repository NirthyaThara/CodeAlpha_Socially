# CodeAlpha_# SocialNetwork

**Live Demo:** [https://codealpha-socially.onrender.com](https://codealpha-socially.onrender.com)

## Project Overview
**SocialNetwork** is a full-stack social media application designed to allow users to connect, share content, and communicate. Users can register, create profiles, post text and images, like/share/save posts, follow other users, and chat with them in real-time.

## Technologies & Languages Used
The project is built using the **MERN Stack**:
- **Frontend**: React (v19), Vite, React Router, CSS Modules.
- **Backend**: Node.js, Express.js.
- **Database**: MongoDB with Mongoose.
- **Tools**: Axios, Multer, JSON Web Tokens.

## Features
- **Authentication**: Secure Login and Registration.
- **Feed**: View posts from all users.
- **Posting**: Create posts with text and images.
- **Interactions**: Like, Share, Save posts.
- **Search**: Search for users.
- **Messaging**: Real-time chat integration.
- **Notifications**: Activity alerts.

## Directory Structure
- **client/**: React frontend application.
- **server/**: Node.js/Express backend API.